...
> **门禁状态：已关闭。** 2026-07-30 的三个 Adapter 脱敏真实 smoke 证据已完成验收，详见 [最终验收记录](../acceptance/adapter-real-smoke.md)。允许进入 Phase 6 实施；Phase 6 尚未在本提交中实现。
...
首次真实 smoke 或后续复验前应确认本地配置已按实际权限要求设置该字段。管理员窗口能够打开程序本身不构成通过证据；2026-07-30 的最终验收还核对了实际运行、正常退出、退出码转发和进程清理。
...
三个 Adapter 的最终脱敏验收证据已固化，门禁已关闭。管理员权限 bootstrap 仍不等于完整工作流，Phase 6 尚未实现。
